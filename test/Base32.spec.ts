...
describe('base32', () => {
   ...
   describe('decode', () => {
      ...
      it('should decode simple examples', () => {
         ...
         expect(Buffer.from(base32Decode('mjswK4TT')).toString()).toBe('beers');
         expect(Buffer.from(base32Decode('MJSWK4TTN5XA====')).toString()).toBe(
            'beerson'
         );
         expect(Buffer.from(base32Decode('MJSWK4TTEAYQ====')).toString()).toBe(
            'beers 1'
         );
         expect(
            Buffer.from(
               base32Decode('ONUG6Y3LNFXGO3DZEBSGS43NNFZXGZLE')
            ).toString()
      ...
      it('should be binary safe', () => {
         expect(
            Buffer.from(
               base32Decode(base32Encode(Buffer.from([0x00, 0xff, 0x88])))
            ).toString('hex')
         ).toBe('00ff88');
         const code =
         ...
         expect(base32Encode(Buffer.from(code, 'hex')).toString()).toBe(
            encoded
         );
